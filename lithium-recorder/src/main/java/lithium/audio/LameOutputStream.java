 ...
 */
public class LameOutputStream extends FilterOutputStream {
	 ...
	 * @param executable LAME executable.
	 *
	 * @throws IOException if an I/O error occurs.
	 */
	public LameOutputStream(OutputStream out, Mp3Options options, File executable)
	        throws IOException {
		super(out);
		...
		Process encoder = runtime.exec(options.getCommandArray(executable));

		encoderInput = encoder.getOutputStream();

		Pipe encoderOutput = new Pipe(encoder.getInputStream(), out);
		Pipe encoderError = new Pipe(encoder.getErrorStream(), System.err);
